"""
CardioRAG Agent — Cardiology Specialist
========================================
...
Usage:
    from CardioRAG.cardio_agent import query_cardio
    result = query_cardio("What is atrial fibrillation?")
"""
...
def load_cardio_documents() -> dict:
    """Dynamically scan and register all Cardio index JSON files."""
    registry = {}
...
DOCUMENTS = load_cardio_documents()
...
def cardio_knowledge_tool(query: str, source_filter: str = None) -> str:
    """Query the CardioRAG Knowledge Base and return structured context string."""
    results = []
    docs_to_search = {
    ...
    if not results:
        return "NO_RESULTS: No relevant cardiology information found."
...
def query_cardio(question: str, history: list = None) -> dict:
    """
    Query the Cardio specialist. Returns structured result dict.
    This is a convenience wrapper — for full power, use ClinicalEngine.

    Returns:
        {"answer": str, "sources": list, "domain": "Cardiology", "out_of_domain": bool}
    """
    import sys
    sys.path.insert(0, str(Path(__file__).parent.parent))
    from clinical_engine import ClinicalEngine
    engine = ClinicalEngine("cardio")
...
if __name__ == "__main__":
    print(f"CardioRAG Agent — Loaded {len(DOCUMENTS)} indexed documents")
    print(f"Index directory: {INDEX_DIR}")
    print()

    test_query = "Atrial fibrillation treatment guidelines"
    print(f"Testing: {test_query}\n")
    result = cardio_knowledge_tool(test_query)
    print(result[:2000])
