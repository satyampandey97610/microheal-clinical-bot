"""
GeriatricRAG Agent — Geriatrics Specialist
==============================================
...
Usage:
    from GeriatricRAG.geriatric_agent import query_geriatric
    result = query_geriatric("What causes GERD?")
"""
...
def load_geriatric_documents() -> dict:
    """Dynamically scan and register all geriatric index JSON files."""
    registry = {}
    ...
    for file in glob.glob(str(INDEX_DIR / "*_structure.json")):
        ...
        if "article" in key_lower:
            doc_type = "Journal"
        elif "978924" in key_lower:
            doc_type = "Guideline"
        elif "978" in key_lower or "bookshelf" in key_lower or "fillit" in key_lower:
            doc_type = "Textbook"
        elif "pocketbook" in key_lower:
            doc_type = "Guide"
...
DOCUMENTS = load_geriatric_documents()
...
def geriatric_knowledge_tool(query: str, source_filter: str = None) -> str:
    """Query the GeriatricRAG Knowledge Base and return structured context string."""
    results = []
    docs_to_search = {
    ...
    if not results:
        return "NO_RESULTS: No relevant geriatrics information found."
...
def query_geriatric(question: str, history: list = None) -> dict:
    """
    Query the geriatric specialist. Returns structured result dict.
    This is a convenience wrapper — for full power, use ClinicalEngine.

    Returns:
        {"answer": str, "sources": list, "domain": "geriatrics", "out_of_domain": bool}
    """
    import sys
    sys.path.insert(0, str(Path(__file__).parent.parent))
    from clinical_engine import ClinicalEngine
    engine = ClinicalEngine("geriatric")
...
if __name__ == "__main__":
    print(f"GeriatricRAG Agent — Loaded {len(DOCUMENTS)} indexed documents")
    print(f"Index directory: {INDEX_DIR}")
    print()

    test_query = "What are the common causes of delirium in the elderly?"
    print(f"Testing: {test_query}\n")
    result = geriatric_knowledge_tool(test_query)
    print(result[:2000])
