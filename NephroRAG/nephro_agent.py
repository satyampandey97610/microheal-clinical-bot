"""
NephroRAG Agent — Nephrology Specialist
=======================================
...
Usage:
    from NephroRAG.nephro_agent import query_nephro
    result = query_nephro("What is CKD-MBD?")
"""
...
def load_nephro_documents() -> dict:
    """Dynamically scan and register all Nephro index JSON files."""
    registry = {}
    ...
    for file in glob.glob(str(INDEX_DIR / "*_structure.json")):
        ...
        if "guideline" in key_lower:
            ...
            doc_type = "Dataset"
        elif "book" in key_lower or "handbook" in key_lower or "student" in key_lower:
            doc_type = "Textbook"
...
DOCUMENTS = load_nephro_documents()
...
def nephro_knowledge_tool(query: str, source_filter: str = None) -> str:
    """Query the NephroRAG Knowledge Base and return structured context string."""
    results = []
    docs_to_search = {
    ...
    if not results:
        return "NO_RESULTS: No relevant nephrology information found."
...
def query_nephro(question: str, history: list = None) -> dict:
    """
    Query the Nephro specialist. Returns structured result dict.
    This is a convenience wrapper — for full power, use ClinicalEngine.

    Returns:
        {"answer": str, "sources": list, "domain": "Nephrology", "out_of_domain": bool}
    """
    import sys
    sys.path.insert(0, str(Path(__file__).parent.parent))
    from clinical_engine import ClinicalEngine
    engine = ClinicalEngine("nephro")
...
if __name__ == "__main__":
    print(f"NephroRAG Agent — Loaded {len(DOCUMENTS)} indexed documents")
    print(f"Index directory: {INDEX_DIR}")
    print()

    test_query = "What is CKD-MBD?"
    print(f"Testing: {test_query}\n")
    result = nephro_knowledge_tool(test_query)
    print(result[:2000])
