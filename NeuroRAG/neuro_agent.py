"""
NeuroRAG Agent — Neurology Specialist
=======================================
...
Usage:
    from NeuroRAG.neuro_agent import query_neuro
    result = query_neuro("What is CKD-MBD?")
"""
...
def load_neuro_documents() -> dict:
    """Dynamically scan and register all Neuro index JSON files."""
    registry = {}
...
DOCUMENTS = load_neuro_documents()
...
def neuro_knowledge_tool(query: str, source_filter: str = None) -> str:
    """Query the NeuroRAG Knowledge Base and return structured context string."""
    results = []
    docs_to_search = {
    ...
    if not results:
        return "NO_RESULTS: No relevant neurology information found."
...
def query_neuro(question: str, history: list = None) -> dict:
    """
    Query the Neuro specialist. Returns structured result dict.
    This is a convenience wrapper — for full power, use ClinicalEngine.

    Returns:
        {"answer": str, "sources": list, "domain": "Neurology", "out_of_domain": bool}
    """
    import sys
    sys.path.insert(0, str(Path(__file__).parent.parent))
    from clinical_engine import ClinicalEngine
    engine = ClinicalEngine("neuro")
...
if __name__ == "__main__":
    print(f"NeuroRAG Agent — Loaded {len(DOCUMENTS)} indexed documents")
    print(f"Index directory: {INDEX_DIR}")
    print()

    test_query = "What is CKD-MBD?"
    print(f"Testing: {test_query}\n")
    result = neuro_knowledge_tool(test_query)
    print(result[:2000])
