"""
OncoRAG Agent — Oncology Specialist
========================================
...
Usage:
    from OncoRAG.onco_agent import query_onco
    result = query_onco("What is immunotherapy?")
"""
...
def load_onco_documents() -> dict:
    """Dynamically scan and register all Onco index JSON files."""
    registry = {}
    ...
    for file in glob.glob(str(INDEX_DIR / "*_structure.json")):
        ...
        if "guideline" in key_lower:
            ...
            doc_type = "Dataset"
        elif "book" in key_lower or "handbook" in key_lower or "manual" in key_lower:
            doc_type = "Textbook"
...
def onco_knowledge_tool(query: str, source_filter: str = None) -> str:
    """Query the OncoRAG Knowledge Base and return structured context string."""
    results = []
    docs_to_search = {
    ...
    if not results:
        return "NO_RESULTS: No relevant oncology information found."
...
def query_onco(question: str, history: list = None) -> dict:
    """
    Query the Oncology specialist. Returns structured result dict.
    This is a convenience wrapper — for full power, use ClinicalEngine.

    Returns:
        {"answer": str, "sources": list, "domain": "Oncology", "out_of_domain": bool}
    """
    import sys
    sys.path.insert(0, str(Path(__file__).parent.parent))
    from clinical_engine import ClinicalEngine
    engine = ClinicalEngine("onco")
...
if __name__ == "__main__":
    # Dynamically reload documents to print correct count on test run
    DOCUMENTS = load_onco_documents()
    print(f"OncoRAG Agent — Loaded {len(DOCUMENTS)} indexed documents")
    print(f"Index directory: {INDEX_DIR}")
    print()

    test_query = "Immunotherapy side effects"
    print(f"Testing: {test_query}\n")
    result = onco_knowledge_tool(test_query)
    print(result[:2000])
