...
async def check_title_appearance_in_start_concurrent(structure, page_list, model=None, logger=None):
    ...
    for node, result in zip(valid_items, results):
        if isinstance(result, Exception):
            node['appear_start'] = 'no'
            if logger:
                logger.error(f"check_title_appearance exception: {result}")
        else:
            if 'appears in the start of the page text' in result:
                node['appear_start'] = 'yes'
            else:
                node['appear_start'] = 'no'
...
def add_page_offset_to_toc_json(data, offset):
    if offset is None:
        # Fallback: cannot compute offset, leave pages as-is but rename to physical_index
...
def process_no_toc(page_list, start_index=1, model=None, logger=None):
    ...
    try:
        with open("live_progress.txt", "w") as f:
            f.write(f"Generating TOC: Part 1 of {len(group_texts)}...\n")
    except:
        pass
    ...
    for i, group_text in enumerate(group_texts[1:]):
        try:
            with open("live_progress.txt", "w") as f:
                f.write(f"Generating TOC: Part {i+2} of {len(group_texts)}...\n")
        except:
            pass
...
def process_toc_no_page_numbers(toc_content, toc_page_list, page_list,  start_index=1, model=None, logger=None):
    ...
    for i, group_text in enumerate(group_texts):
        try:
            with open("live_progress.txt", "w") as f:
                f.write(f"Matching TOC to Pages: Part {i+1} of {len(group_texts)}...\n")
        except:
            pass
...
async def verify_toc(page_list, list_result, start_index=1, N=None, model=None):
    ...
    for result in results:
        if isinstance(result, Exception):
            incorrect_results.append({'answer': 'no', 'exception': True})
            if logger:
                logger.error(f"check_title_appearance exception in check_toc: {result}")
        else:
...
async def meta_processor(page_list, mode=None, toc_content=None, toc_page_list=None, start_index=1, opt=None, logger=None):
    ...
    if accuracy > 0.6 and len(incorrect_results) > 0:
        ...
        print('Processing accuracy low, but continuing for high textual coverage...')
        return toc_with_page_number
...
async def process_large_node_recursively(node, page_list, opt=None, logger=None):
    ...
    if 'nodes' in node and node['nodes']:
        ...
        # Rate limit by chunking
        chunk_size = 3
        for i in range(0, len(tasks), chunk_size):
            chunk = tasks[i:i + chunk_size]
            chunk_results = await asyncio.gather(*chunk, return_exceptions=True)
            for r in chunk_results:
                if isinstance(r, Exception) and logger:
                    logger.error(f"Error in recursive chunk: {r}")
            if i + chunk_size < len(tasks):
...
async def tree_parser(page_list, opt, doc=None, logger=None):
    ...
    for i in range(0, len(tasks), chunk_size):
        try:
            with open("live_progress.txt", "w") as f:
                f.write(f"Deep Processing Sections: Chunk {i//chunk_size + 1} of {(len(tasks) + chunk_size - 1)//chunk_size}...\n")
        except:
            pass
        chunk = tasks[i:i + chunk_size]
        chunk_results = await asyncio.gather(*chunk, return_exceptions=True)
        for r in chunk_results:
            if isinstance(r, Exception) and logger:
                logger.error(f"Error in deep processing chunk: {r}")
        if i + chunk_size < len(tasks):
            await asyncio.sleep(1)
...
def page_index_main(doc, opt=None):
    ...
    try:
        with open("live_progress.txt", "w") as f:
            f.write(f"Loading Phase 1: {pdf_name}...\n")
    except:
        pass
