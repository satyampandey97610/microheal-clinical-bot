...
RESULTS_DIR = root_dir / "CardioRAG" / "index"
LOGS_DIR = root_dir / "logs"
TOKEN_LOG_PATH = LOGS_DIR / "token_usage_master.json"
AGENT_PATH = root_dir / "CardioRAG" / "cardio_agent.py"
...
def generate_registry(processed_files):
    """Generate the Python dictionary string for cardio_agent.py"""
    registry_entries = []
    for pdf_name, json_name in processed_files:
        key_name = pdf_name.lower().replace(" ", "_").replace("-", "_").replace(".", "_")
        entry = f'''    "{key_name}": {{
        "tree_path": "./CardioRAG/index/{json_name}",
        "pdf_path":  "./CardioRAG/data/{pdf_name}",
        "description": "Cardiology Reference: {pdf_name}",
        "year": 2024,
        "type": "guideline"
    }}'''
...
def update_agent_registry(processed_files):
    """Automatically update the DOCUMENTS dictionary in cardio_agent.py"""
    with open(AGENT_PATH, 'r', encoding='utf-8') as f:
        content = f.read()

    new_dict = generate_registry(processed_files)
    
    # Regex to find DOCUMENTS = { ... } and replace it
    pattern = re.compile(r'DOCUMENTS\s*=\s*\{.*?\}', re.DOTALL)
    new_content = pattern.sub(f'DOCUMENTS = {new_dict}', content)
    
    with open(AGENT_PATH, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print(f"[SUCCESS] Updated DOCUMENTS registry in {AGENT_PATH.name}")

def main():
    print(f"\n--- CardioRAG Batch Indexer ---")
    pdfs = [PDF_DIR / "arnett-et-al-2019-2019-acc-aha-guideline-on-the-primary-prevention-of-cardiovascular-disease-a-report-of-the-american.pdf"]
    
    if not pdfs:
        print(f"[ERROR] No PDFs found in {PDF_DIR}")
        return

    print(f"Found {len(pdfs)} documents to index. Estimated time: 30-45 minutes.\n")
    
    success, failed = 0, 0
    processed_files = [] # (pdf_name, json_name)
    ...
    for pdf_path in pdfs:
        ...
        if output_file.exists():
            ...
            processed_files.append((pdf_path.name, output_file.name))
            continue
    ...
    if processed_files:
        print("Updating Agent Registry...")
        update_agent_registry(processed_files)
