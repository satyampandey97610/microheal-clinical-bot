...
if api_key:
    print(f"DEBUG: PageIndex API Key found: {api_key[:10]}...")
    litellm.api_key = api_key
else:
    print("DEBUG: PageIndex API Key NOT FOUND in environment!")
...
def llm_completion(model, prompt, chat_history=None, return_finish_reason=False):
    ...
    for i in range(max_retries):
        try:
            ...
            wait_time = min(2**i + 5, 60) # Exponential backoff with a base of 5s
            print(f"************* Rate Limit Hit (Completion). Waiting {wait_time}s... *************")
...
async def llm_acompletion(model, prompt):
    ...
    for i in range(max_retries):
        try:
            ...
            wait_time = min(2**i + 5, 60)
            print(f"************* Rate Limit Hit (ACompletion). Waiting {wait_time}s... *************")
...
def extract_json(content):
    import re
    try:
        # Check if the LLM hallucinated multiple JSON blocks or stitched objects
        # e.g., { "table_of_contents": [...] } { "table_of_contents": [...] }
        # We can find all blocks that look like a full object and parse them individually
        
        # Strip code block wrappers if any
        if "```json" in content:
            # Join all json blocks into one string just in case
        ...
        json_content = content.replace('None', 'null')
        ...
        # If direct parse fails, try to heal stitched arrays or multiple objects
        # Because we normalized whitespace, there might be a space between brackets
        json_content = json_content.replace('} {', '},{').replace('] [', '],[')
        json_content = json_content.replace('}{', '},{').replace('][', '],[')
        
        # If it's a stitched object like { "k": [...] },{ "k": [...] }
        if json_content.startswith('{') and '},{' in json_content:
            try:
                ...
                for obj in parsed_list:
                    for k, v in obj.items():
                        if k not in combined:
                            combined[k] = v
                        elif isinstance(combined[k], list) and isinstance(v, list):
                            combined[k].extend(v)
                return combined
            except json.JSONDecodeError:
                pass
                
        # Try cleanup for trailing commas
        json_content = json_content.replace(',]', ']').replace(',}', '}')
        return json.loads(json_content)
        ...
        for i in range(len(data)):
            ...
            if isinstance(data[i], dict) and 'physical_index' in data[i]:
                ...
                if isinstance(val, str):
                    ...
                    if match:
                        data[i]['physical_index'] = int(match.group())
                    else:
