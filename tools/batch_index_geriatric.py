"""
GeriatricRAG Batch Indexer — Production-Grade
==========================================
...
# ── Directories ───────────────────────────────────────────────────────────────
PDF_DIR          = root_dir / "GeriatricRAG" / "data"
RESULTS_DIR      = root_dir / "GeriatricRAG" / "index"
LOGS_DIR         = root_dir / "logs"
TOKEN_LOG_PATH   = LOGS_DIR / "token_usage_master.json"
COST_REPORT_PATH = LOGS_DIR / "geriatric_cost_report.md"
...
# ── Verified Smart Order (small → large) ──────────────────────────────────────
INDEXING_ORDER = [
    ("41599_2023_Article_1629.pdf", 12, "journal"),
    ("9789241513487-eng.pdf", 27, "guideline"),
    ("L-G-0000001100-0002331378.pdf", 30, "reference"),
    ("978-1-62808-976-9_ch15.pdf", 31, "textbook"),
    ("NGIG Geriatrics Pocketbook 2025.pdf", 50, "guide"),
    ("9789240033245-eng.pdf", 137, "guideline"),
    ("9789240694811_eng.pdf", 260, "guideline"),
    ("Bookshelf_NBK379406.pdf", 332, "textbook"),
    ("Howard_M._Fillit_MD_Kenneth_Rockwood_Kenneth_Wo.pdf", 1160, "textbook"),
]
...
def run_preflight() -> bool:
    ...
    # Directories
    for d, label in [(PDF_DIR, "GeriatricRAG/data"), (LOGS_DIR, "logs")]:
        if d.exists():
...
def write_cost_report(session_data: dict, failed_pdfs: list):
    ...
    page_ref = {fname.replace('.pdf', ''): pages for fname, pages, _ in INDEXING_ORDER}
    ...
    lines += [
        ...
        f"- This report   : `logs/geriatric_cost_report.md`",
    ]
...
def main():
    log("=" * 62)
    log("  GeriatricRAG Batch Indexer - Production Grade")
    ...
    for idx, (pdf_path, exp_pages, doc_type) in enumerate(pdfs, 1):
        ...
        # Write live progress for real-time monitoring
        try:
            with open("live_progress.txt", "w") as f:
                f.write(f"=== INDEXING: {pdf_path.name} ===\n")
                f.write(f"PDF {idx}/{total} | Type: {doc_type} | Pages: {exp_pages}\n")
                f.write(f"Started: {datetime.now().strftime('%H:%M:%S')}\n")
                f.write(f"Session cost so far: ${session_running_cost:.4f}\n\n")
        except Exception:
            pass
    ...
    log("")
    log("=" * 62)
    log("  GeriatricRAG Indexing Complete")
    log("=" * 62)
        ...
        try:
            ...
            for s in log_data.get("all_sessions", []):
                if s.get("session_start") == SESSION_START:
                    ...
                    t = s.get("session_totals", {})
                    log("")
                    log("  THIS SESSION — GeriatricRAG Cost Summary")
                    log("  " + "-" * 48)
